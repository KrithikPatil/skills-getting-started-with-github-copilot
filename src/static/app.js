document.addEventListener("DOMContentLoaded", () => {
  ...
  async function fetchActivities() {
    try {
              ...
              <ul class="participants-list no-bullets">
                ${details.participants.map(email => `
                  <li data-activity="${encodeURIComponent(name)}" data-email="${encodeURIComponent(email)}">
                    ...
                    <span class="delete-participant" title="Remove participant" style="cursor:pointer; color:#c62828; margin-left:8px;">🗑️</span>
                  </li>
                `).join("")}
              </ul>
            </div>
      ...
      // Add delete handler for participants (after DOM update)
